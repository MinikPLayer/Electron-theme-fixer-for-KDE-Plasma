#!/usr/bin/env python3
"""Watches ~/.config/gtk-3.0 and ~/.config/gtk-4.0 for changes and prints
them to the console.

Quick debugging helper - not part of the fixer itself. Uses the `watchdog`
package (inotify-backed on Linux).
"""
...
# The specific files we can detect the *actual* completion of - a write-close
# (IN_CLOSE_WRITE) or an atomic rename landing on them - rather than just
# guessing based on a quiet period. See DebouncedHandler.
EXPECTED_SETTINGS_FILES = {directory / "settings.ini" for directory in WATCHED_DIRS}

# Upper bound to wait for a change we can't positively confirm is finished -
# e.g. unrelated file churn in the watched directories, or (belt-and-braces)
# a settings.ini write whose completion event never arrives for some reason.
# Reset on every new event. Only actually elapses if nothing conclusively
# signals "done" first - see on_any_event.
...
class DebouncedHandler(FileSystemEventHandler):
    ...
    def __init__(self, on_settled, debounce_seconds: float = DEBOUNCE_SECONDS):
    ...
    def on_any_event(self, event):
        ...
        #     print(f"[{event.event_type}] {kind}: {event.src_path} -> {event.dest_path}")
        # else:
        #     print(f"[{event.event_type}] {kind}: {event.src_path}")
    ...
    @staticmethod
    def _settings_ini_path(event) -> Path | None:
        if event.is_directory:
            return None
        for path_str in (event.src_path, getattr(event, "dest_path", None)):
            if path_str and Path(path_str) in EXPECTED_SETTINGS_FILES:
...
def on_settled(settings_modified: bool) -> None:
    print(f"Done! (settings_modified={settings_modified})")
    if settings_modified:
        time.sleep(0.05)
        print("Settings.ini changed - notifying dconf writer")
        subprocess.run(
            [
                gdbus, "emit", "--session",
...
def main() -> int:
    observer = Observer()
    handler = DebouncedHandler(on_settled)
